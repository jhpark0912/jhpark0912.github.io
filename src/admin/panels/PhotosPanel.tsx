...
import { defaultContent } from '../../lib/siteConfig'
 ...
 */
function uploadedBytes(content: { gallery: GalleryPhoto[]; cover: { photoId?: string; image: string } }): number {
  const gallery = content.gallery.reduce((sum, photo) => sum + (photo.photoId ? photo.src.length : 0), 0)
  return gallery + (content.cover.photoId ? content.cover.image.length : 0)
}
